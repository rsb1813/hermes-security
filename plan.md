...
## Current Protocol-v5 factorized-flow checkpoint

- The original explicit-row Protocol 5 implementation and workflow integration are complete, but its balanced selector retained zero exact critical, entry, and pair tasks on the held-out host gate. Reordering, critical bundles, confidence, frontier priority, and per-pair compression did not create a production-budget exact pair.
- Replace only the Protocol 5 packet representation with canonical path and component dictionaries, a graph-derived entry bank, a critical bank that also preserves original standalone critical roots, and at most four deterministic entry adjacencies per critical. Keep the artifact path, two-read discovery boundary, two-call ceiling, verification projection, response schemas, scoring, frontier, sandbox, network denial, and Protocol 1 through Protocol 4 bytes unchanged.
- Use 64 graph entries for `hunt-balanced`, order criticals by selected semantic operation rank and canonical key, and retain the largest prefix that fits 128 rows, 65,536 bytes, and 1,024 bytes per row. `hunt-max` doubles entries, rows, and bytes while keeping the same deterministic algorithm.
- Attest `join-e<entry_id>-c<critical_id>` only for an exact retained adjacency and `sink-c<critical_id>` only for an exact retained critical. Preserve at most four non-reserved whole-frontier fallbacks and require source inspection because neither bank membership nor adjacency is proof.
- The diagnostic hybrid retained exact critical coverage at two tasks, increased exact entries from zero to two and exact pairs from zero to one, and reduced aggregate model-visible bytes from the Protocol 4 control's 324,248 to 262,050. This passes the host input-quality gate but does not prove model recall.
- Implement the amendment with public RED/GREEN tests, reproduce it from the pinned snapshot, rerun the complete no-model and legacy compatibility gates, and request a paid Canary only after a clean independent review. No model, network, container, or paid invocation is authorized in this unit.
- The first product build preserved two exact critical tasks but retained only one exact entry and no exact pair. A four-way aggregate A/B diagnostic isolated the loss to the approximate graph-entry representative rather than adjacency ordering.
- The corrected product builder reuses the complete canonical graph-entry ordering only for deterministic representative selection. Its pinned eight-task host gate retained two exact critical tasks, two exact entry tasks, one exact pair, and four exact trace nodes in 262,028 aggregate model-visible bytes. Snapshot integrity and all eight priority and semantic hashes matched. This is verified host-input coverage, not end-to-end model recall.
