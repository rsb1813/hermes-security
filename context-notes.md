...
- Task 5 uses the pinned Codex `0.150.0-alpha.8` external `chatgptAuthTokens` file shape only as a version-locked compatibility boundary. It passes the access token and account ID with an empty refresh token and never copies the host auth file or Codex home.
...
- Multi-agent capability remains explicitly enabled and identically bounded for both benchmark arms. Disabling it would prevent Standard's independent audit worker and would make Hunt's distinct verifier identity impossible, directly lowering discovery and validation quality. The frozen controls include subagent model, effort, concurrency, and depth, while JSONL collaboration payloads are scrubbed to event-type counts.
