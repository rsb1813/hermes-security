...
#### Task 2A: Recover the first live artifact-gate failure before performance acceptance

The first live artifact-gate invocation reached a terminal model response, then failed post-runtime attestation with only the broad `hunt_evidence_invalid` code. Its discovery-failure workflow receipt also failed independent validation because the validator required a complete predictions row set even when the committed task receipt was failed. Do not inspect or retain the discarded model response, source locations, candidate prose, work IDs, or raw command arguments.

- [ ] **Step 1: Add a RED workflow test for discovery failure receipt validation.** The fixture must contain a failed discovery task, empty predictions, empty Hunt evidence, empty candidate transfer, no verification phase, and a valid failure sidecar. Validation must accept the exact committed receipt and reject a non-empty candidate transfer, unexpected evidence row, or completed-task prediction mismatch.
- [ ] **Step 2: Make incomplete discovery validation follow task receipts.** Validate predictions and Hunt evidence only for completed discovery task records in manifest order. When the discovery phase is not completed, require the candidate transfer to be empty, the workflow status to be incomplete, verification and public-prediction fields to be absent, and all existing phase/failure hashes to match.
- [ ] **Step 3: Add RED adapter and runner tests for bounded attestation categories.** Cover missing or duplicate exact packet reads, prepared-artifact integrity failure, candidate location outside the frontier, and candidate search-pass mismatch. Failure artifacts must contain only one allowlisted code and must never contain an exception message, path, preview, candidate text, hash, or task identity.
- [ ] **Step 4: Add typed Hunt evidence categories and preserve the existing broad fallback.** Classify at the failing attestation boundary rather than parsing exception strings. Keep all preparation, parsing, and unknown failures on `hunt_evidence_invalid`; expose only fixed path-free public codes for the four live attestation boundaries.
- [ ] **Step 5: Verify and commit in two logical units.** First commit discovery-failure receipt revalidation. Then commit bounded Hunt attestation diagnostics. Run focused RED/GREEN tests, the full HermesBench Python suite, the combined bundled-skill Bun tests, compileall, and `git diff --check`.
- [ ] **Step 6: Revalidate the retained failed v12 receipt, repeat no-model boundary checks, and run one paid v12b diagnostic under a new output root.** The failed v12 result is harness evidence only and must not enter the v10/v11 performance comparison.

#### Task 2B: Preserve the command-audit boundary while identifying the v12b rejection origin

The v12b discovery runtime ended before attestation with the existing `command_event_invalid` code. The retained receipt revalidated, the snapshot remained unchanged, all success artifacts were empty, no authentication or host-path value was retained, and no container remained. Do not broaden command normalization without identifying the exact rejected syntax class.

- [ ] **Step 1: Add RED tests for every scanner and parser origin.** Cover non-string command shape, empty/NUL input, newline, unquoted pipe, redirect, control operator, grouping, substitution, comment, double-quoted substitution, dangling escape or malformed quote, shell parse failure, excessive wrapper depth, and unencodable token.
- [ ] **Step 2: Assign fixed public codes at the exact rejection origin.** Preserve `command_event_invalid` as the broad legacy fallback. New codes may reveal only the static syntax class; they must never include the raw command, argument, path, regex, operator text, exception message, task ID, or a digest derived from confidential text.
- [ ] **Step 3: Keep rejection semantics byte-for-byte equivalent.** Quoted search metacharacters remain accepted. Every existing pipeline, redirect, control operator, substitution, newline, malformed quoting, excessive wrapper, and encoding rejection remains rejected before success artifacts are written. Standard uses the same parser and must not otherwise change.
- [ ] **Step 4: Bind every code through the existing failure evidence and receipt path.** Add allowlist coverage, canonical one-code `failure.json` tests, legacy-code compatibility, atomic success-artifact cleanup, snapshot immutability, and incomplete workflow receipt revalidation.
- [ ] **Step 5: Run focused RED/GREEN tests, the complete HermesBench Python suite, the combined bundled-skill Bun tests, compileall, and `git diff --check`; self-review and commit one logical unit.**
- [ ] **Step 6: Revalidate retained v12b, repeat the no-model boundary checks, then run exactly one paid v12c diagnostic under a new output root.** Only a concrete fixed code may justify a later narrowly scoped normalization change.
